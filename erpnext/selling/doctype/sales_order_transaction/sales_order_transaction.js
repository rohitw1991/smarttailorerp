...
erpnext.selling.SalesOrderController = erpnext.selling.SellingController.extend({
	refresh: function(doc, dt, dn) {
		...
		this.order_type(doc);
		this.make_merchandise_items(doc);
	},
	...
	make_material_request: function() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.sales_order_transaction.sales_order_transaction.make_material_request",
			frm: cur_frm
		})
	},
	...
	make_delivery_note: function() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.sales_order_transaction.sales_order_transaction.make_delivery_note",
			frm: cur_frm
		})
	},
	...
	make_sales_invoice: function() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.sales_order_transaction.sales_order_transaction.make_sales_invoice",
			frm: cur_frm
		})
	},
	...
	make_maintenance_schedule: function() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.sales_order_transaction.sales_order_transaction.make_maintenance_schedule",
			frm: cur_frm
		})
	},
	...
	make_maintenance_visit: function() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.sales_order_transaction.sales_order_transaction.make_maintenance_visit",
			frm: cur_frm
		})
	},
	make_merchandise_items: function() {
		// console.log("in the js")
		// var cl=doc.merchandise_items;
		// console.log(cl);
...
cur_frm.cscript.m_qty = function(doc, cdt, cdn){
	cur_frm.cscript.calculate_total_amt(doc, cdt, cdn)
	
}

cur_frm.cscript.m_rate = function(doc, cdt, cdn){
	cur_frm.cscript.calculate_total_amt(doc, cdt, cdn)
	
}

cur_frm.cscript.calculate_total_amt = function(doc, cdt, cdn){
	var d = locals[cdt][cdn]
	value = (parseFloat(d.m_qty) * parseFloat(d.m_rate))
	if (value){
		d.amount = value.toString()
		doc.net_total_export = (parseFloat(doc.net_total_export)+ parseFloat(value)).toString()
	}
	refresh_field(["net_total_export","merchandise_items"])
}