...
class SellingControllerTransaction(StockController):
	def __setup__(self):
		frappe.errprint("in the Sellin Controller Transaction2")
	...
	def validate(self):
		super(SellingControllerTransaction, self).validate()
	...
	def set_missing_values(self, for_validate=False):
		super(SellingControllerTransaction, self).set_missing_values(for_validate)
	...
	def calculate_taxes_and_totals(self):
		...
		super(SellingControllerTransaction, self).calculate_taxes_and_totals()
	...
	def calculate_item_values(self):
		if not self.discount_amount_applied:
			frappe.errprint("in the calculate_item_values")
	...
	def calculate_net_total(self):
		...
		frappe.errprint("self.entries")
		net_total1=net_total_export1=0.0

		for item1 in self.get("entries"):
			net_total1 += item1.rate
			net_total_export1 += item1.amount
			frappe.errprint(item1.rate)
			frappe.errprint(item1.item_code)
		frappe.errprint("net_total1")	
		frappe.errprint(net_total1)
		frappe.errprint(net_total_export1)
		...
		self.net_total_export=net_total1+self.net_total
		frappe.errprint(self.net_total_export)	
